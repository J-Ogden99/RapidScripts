...
def gen_inflow_namelist(rapid_inputs: str,
    ...
    for path in (rapid_inflows, rapid_discharge, namelist_dir):
    ...
    for start_date, end_date in zip(
        ...
        if len(glob(os.path.join(rapid_inflows, f"m3*{start_date_code}_{end_date_code}.nc"))) == 0:
        ...
        if int(start_date.year) == start_year:
            ...
            qinit_file = os.path.join(rapid_discharge, f"Qfinal_{vpu_id}_{last_step_date_code}.nc")
